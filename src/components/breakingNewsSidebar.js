...
class BreakingNewsSidebar extends React.Component {
     ...
     componentDidMount() {
        ...
        this.setState({ loading: true }, () => {
            axios
            .get(`${wordPressSiteUrl}/wp-json/wp/v2/posts/?categories=3&per_page=5`)
                .then(res => {
       ...
       { posts.length ? (
        ...
        { posts.map( post =>(
            <div key={ post.id }  id="blogrow" className="row" style={{ backgroundColor: 'rgb(50,52,54)', marginBottom: '10px', margin: '5px'}}>
                <div className="col-4">
                  <img  className="postimg" src={ post.better_featured_image.source_url }   style={{ width: '100px', height: '100px', marginBottom: '10px', marginTop: '15px', borderRadius: '10px' }} />
                </div>
                    <div className="col-8 blogsidebar col-8">
